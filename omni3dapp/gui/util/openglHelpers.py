...
# Hack; required before glut can be called.
# Not required for all OS.
glutInit()
    ...
    def __init__(self, vertexProgram, fragmentProgram):
        ...
        try:
            vertexShader = shaders.compileShader(
                vertexProgram, GL_VERTEX_SHADER)
            fragmentShader = shaders.compileShader(
                fragmentProgram, GL_FRAGMENT_SHADER)

            """
            shader.compileProgram tries to return the shader program
            as an overloaded int. But the return value of a shader does not
            always fit in a int (needs to be a long).
            So we do raw OpenGL calls.
            This is to ensure that this works on intel GPU's.
            self._program = shaders.compileProgram(
                self._vertexProgram, self._fragmentProgram)
            """
    ...
    def setUniform(self, name, value):
        if self._program is not None:
            if type(value) is float:
                ...
                glUniformMatrix3fv(
                    glGetUniformLocation(self._program, name),
                    1, False, value.getA().astype(numpy.float32))
            else:
                log.error('Unknown type for setUniform: {0}'.format(
                    (str(type(value)))))
...
class GLVBO(GLReferenceCounter):
    ...
    """
    def __init__(self, renderType, vertexArray, normalArray=None,
                 indicesArray=None):
        super(GLVBO, self).__init__()
        # TODO: Add size check to see if normal and vertex arrays
        # have same size.
        ...
        if not bool(glGenBuffers):  # Fallback if buffers are not supported.
            ...
            if self._hasIndices:
                ...
                glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                             numpy.array(indicesArray, numpy.uint32),
                             GL_STATIC_DRAW)
    ...
    def render(self):
        ...
        if self._buffers is None:
            ...
            if self._hasIndices:
                glDrawElements(self._renderType, self._size, GL_UNSIGNED_INT,
                               self._indicesArray)
            else:
                # Warning, batchSize needs to be dividable by 4 (quads),
                # 3 (triangles) and 2 (lines). Current value is magic.
                batchSize = 996
                # leftovers.
            ...
            for info in self._buffers:
                ...
                if self._hasIndices:
                    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self._bufferIndices)
                    glDrawElements(self._renderType, self._size,
...
def glDrawStretchedQuad(x, y, w, h, cornerSize, texID):
    """
    Same as draw texured quad, but without stretching the corners.
    Useful for resizable windows.
    """
    tx0 = float(texID % 4) / 4
...
def unproject(winx, winy, winz, modelMatrix, projMatrix, viewport):
    ...
    """
    npModelMatrix = numpy.matrix(numpy.array(modelMatrix,
                                             numpy.float64).reshape((4, 4)))
    npProjMatrix = numpy.matrix(numpy.array(projMatrix,
                                            numpy.float64).reshape((4, 4)))
...
def convert3x3MatrixTo4x4(matrix):
    ret = []
    ret.extend(list(matrix.getA()[0]))
    ret.append(0)
    ret.extend(list(matrix.getA()[1]))
    ret.append(0)
    ret.extend(list(matrix.getA()[2]))
    ret.extend([0, 0, 0, 0, 1])
    return ret
