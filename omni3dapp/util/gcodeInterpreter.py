...
class GCode(object):
    ...
    def __init__(self, sceneview):
        ...
        # self.progress_update_sig = ProgressUpdate()
        self.parse_line_timer = QtCore.QTimer(self._sceneview)
    ...
    def _load(self, gcodeFile):
        ...
        self.extrudeAmountMultiply = 1.0
        self.absoluteE = True
        self.scale = 1.0
        self.posAbs = True
        self.moveType = 'move'
        ...
        self.currentLayer = []
        self.currentPath = gcodePath('move', self.pathType, self.layerThickness, self.pos)
        self.currentPath['extruder'] = self.currentExtruder

        self.currentLayer.append(self.currentPath)

        print "setting timer shot"
        self.parse_line_timer.singleShot(0, self.parse_line)

    def after_parsing_file(self):
        ...
        if self.progressCallback is not None and self._fileSize > 0:
            self.progressCallback(float(self.gcodeFile.tell()) / \
                    float(self._fileSize), self.layerList)
        self._sceneview.updateGL()

    def parse_line(self):
        line = self.gcodeFile.readline()
        if not line:
            return self.after_parsing_file()
        ...
        if ';' in line:
            ...
            if comment.startswith('LAYER:'):
                ...
                for path in self.currentLayer:
                    ...
                    path['extrusion'] = numpy.array(path['extrusion'], numpy.float32)
                self.layerList.append(self.currentLayer)
                if self.progressCallback is not None and \
                    self.progressCallback(float(self.gcodeFile.tell()) / \
                            float(self._fileSize), self.layerList):
        ...
        if G is not None:
            if G == 0 or G == 1:    #Move
                ...
                if self.posAbs:
                    if x is not None:
                        self.pos[0] = x * self.scale + self.posOffset[0]
                    if y is not None:
                        self.pos[1] = y * self.scale + self.posOffset[1]
                    if z is not None:
                        self.pos[2] = z * self.scale + self.posOffset[2]
                else:
                    if x is not None:
                    ...
                    if z is not None:
                        self.pos[2] += z * self.scale
                self.moveType = 'move'
                if e is not None:
                    if self.absoluteE:
                        e -= self.currentE
                    if e > 0.0:
                        self.moveType = 'extrude'
                    if e < 0.0:
                        self.moveType = 'retract'
                    self.currentE += e
                else:
                    e = 0.0
                if self.moveType == 'move' and oldPos[2] != self.pos[2]:
                    if oldPos[2] > self.pos[2] and abs(oldPos[2] - \
                            self.pos[2]) > 5.0 and self.pos[2] < 1.0:
                        oldPos[2] = 0.0
                ...
                if self.currentPath['type'] != self.moveType or \
                    ...
                    self.currentPath = gcodePath(self.moveType,
                            self.pathType, self.layerThickness, self.currentPath['points'][-1])
                    self.currentPath['extruder'] = self.currentExtruder
                    self.currentLayer.append(self.currentPath)
