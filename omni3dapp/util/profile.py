...
setting('machine_width', '210.00', float, 'machine', _('Machine dimensions')
        ).setLabel(_("Maximum width (mm)"), _("Size of the machine in mm")).setRange(0, 2000)
setting('machine_depth', '210.00', float, 'machine', _('Machine dimensions')
        ).setLabel(_("Maximum depth (mm)"), _("Size of the machine in mm")).setRange(0, 2000)
setting('machine_height', '240.00', float, 'machine', _('Machine dimensions')
...
def getBasePath():
    ...
    if not os.path.isdir(basePath):
        try:
            ...
            log.error("Failed to create directory: %s" % (basePath))
    return basePath

def getAlternativeBasePaths():
    """
    Search for alternative installations of the app and their preference files.
    Used to load configuration from older versions of the app.
    """
    paths = []
    ...
    for subPath in os.listdir(basePath):
        ...
        if os.path.isdir(path) and os.path.isfile(os.path.join(path, 'preferences.ini')) and path != getBasePath():
            paths.append(path)
        path = os.path.join(basePath, subPath, 'omni3dapp')
        if os.path.isdir(path) and os.path.isfile(os.path.join(path, 'preferences.ini')) and path != getBasePath():
            paths.append(path)
    return paths
...
def loadProfile(filename, allMachines = False):
    ...
    try:
        ...
        log.error("Parsing error: {0}".format(0))
        return
    if allMachines:
        n = 0
...
def loadPreferences(filename):
    ...
    for setting in settingsList:
        if setting.isPreference():
            if profileParser.has_option('preference', setting.getName()):
                setting.setValue(unicode(profileParser.get('preference', setting.getName()), 'utf-8', 'replace'))
...
def getPreference(name):
    ...
    if name in settingsDictionary and settingsDictionary[name].isPreference():
        return settingsDictionary[name].getValue()
    traceback.print_stack()
    msg = 'Error: "%s" not found in preferences\n' % (name)
    log.error(msg)
...
def getMachineSetting(name, index = None):
    ...
    traceback.print_stack()
    msg = 'Error: "%s" not found in machine settings\n' % (name)
    log.error(msg)
    sys.stderr.write(msg)
    return ''
